...
data_07 <- read_excel("inputs/final_group_wise_bursa_lesion_scores.xlsx", sheet = "07 dpi")
data_21 <- read_excel("inputs/final_group_wise_bursa_lesion_scores.xlsx", sheet = "21 dpi")
data_35 <- read_excel("inputs/final_group_wise_bursa_lesion_scores.xlsx", sheet = "35 dpi")
...
categorize_scores <- function(score) {
  case_when(
    score <= 1 ~ "Normal",
    score >= 2 & score <= 3 ~ "Mild",
    score >= 4 & score <= 7 ~ "Moderate",
    score >= 8 & score <= 10 ~ "Severe"
  )
}
...
sink("outputs/bursa_fisher_exact_test_results.txt")
...
ggplot(data_combined, aes(x = Group, fill = factor(score_category))) +
  ...
  scale_fill_manual(values = rev(viridis::viridis(4, option = "D"))) +  # Use a nice gradient from viridis
  # Change legend title
  guides(fill = guide_legend(title = "Bursal Lesions")) +
...
ggsave("outputs/bursa_lesion_fisher.svg", width = 12, height = 8, dpi = 600)
